...
void BGM::GetMusicPosition(double &position) const {
    if (m_BGM) {
        position = Mix_GetMusicPosition(m_BGM.get());
    } else {
        position = 0;
    }
}
...
void BGM::PlayMusicInPosition(double position){
    if(m_BGM) {
        Mix_SetMusicPosition(position);
        Mix_PlayMusic(m_BGM.get(), -1);
    } else {
        LOG_DEBUG("No music loaded to play at position: {}", position);
    }
}
