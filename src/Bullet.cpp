...
Bullet::Bullet(float start_x, float start_y, float speed, bool is_slow_down, std::string png_name)
 ...
 void Bullet::Update()
{
   ...
   // LOG_INFO("Bullet::Update {0}, {1}", m_Transform.translation.x, m_Transform.translation.y);
    ...
    if (PVZGame::GetInstance().CheckBulletCollisions(m_Transform.translation.x, m_Transform.translation.y, m_is_slow_down)) {
        m_destroyed = true;
        SetVisible(false);
       // LOG_INFO("Bullet::Update {0}, {1}", m_Transform.translation.x, m_Transform.translation.y);
   }
